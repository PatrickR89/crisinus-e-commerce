...
import { CHANGE_CURRENCY } from "../actions/currency_actions";

const eurToKn = 7.513562;
...
export const CurrencyProvider = ({ children }) => {
  ...
  const formatPriceEur = (number) => {
    const inEur = (number / 100 / eurToKn).toFixed(2);
  ...
  useEffect(() => {
    localStorage.setItem("currency", JSON.stringify(state.current_currency));
  }, [state.current_currency]);

  return (
    <CurrencyContext.Provider value={{ ...state, updateCurrency, priceFormat }}>
      {children}
